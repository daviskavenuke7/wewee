...
const FASTLIPA_API_URL = (process.env.FASTLIPA_API_URL || 'https://api.fastlipa.com').replace(/\/+$/, '');
const FASTLIPA_API_KEY = process.env.FASTLIPA_API_KEY;
const MOCK_PAYMENTS = process.env.MOCK_PAYMENTS === 'true';
const FASTLIPA_BASE_URL = FASTLIPA_API_URL.endsWith('/v1') ? FASTLIPA_API_URL : `${FASTLIPA_API_URL}/v1`;
...
export async function createFastLipaPayment(
  ...
  try {
    ...
    const endpoint = `${FASTLIPA_BASE_URL}/transaction/create`;
    const payload = {
      number: phone,
      amount: input.amount_tsh,
      reference,
    ...
    if (isHTML) {
      throw new Error(`Wrong FastLipa endpoint or wrong request format. Got HTML response instead of JSON. Check FASTLIPA_API_URL: ${FASTLIPA_BASE_URL}`);
    }
    ...
    // FastLipa returns tranID in the success envelope data payload.
    const fastLipaTransactionId = data.data?.tranID || data.data?.tranid;
    ...
    return {
      success: true,
      reference: fastLipaTransactionId,
      status: 'pending',
...
export async function verifyFastLipaPayment(
  ...
  try {
    ...
    // PRODUCTION: Call real FastLipa API
    const endpoint = `${FASTLIPA_BASE_URL}/transaction/status?tranid=${encodeURIComponent(reference)}`;
    ...
    if (isHTML) {
      const error = new Error(
        ...
        `Check FASTLIPA_API_URL: ${FASTLIPA_BASE_URL}/transaction/status . ` +
    ...
    // FastLipa uses a payment_status field in the transaction status response.
    const fastLipaStatus = data.data?.payment_status ?? data.data?.status;
    if (!fastLipaStatus) {
